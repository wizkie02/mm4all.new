...
const CursorEffect = () => {
  ...
  useEffect(() => {
    ...
    <CursorContainer 
      ref={cursorRef}
      className={`${isVisible ? 'visible' : 'hidden'} ${isClicked ? 'clicked' : ''} ${isHovered ? 'hovered' : ''}`}
    >
      <CursorInnerGlow />
      <CursorGlow 
        isClicked={isClicked}
...
const CursorContainer = styled.div`
  ...
  top: 0;
  left: 0;
  width: 31.5px; /* Reduced from 35px to 90% */
  height: 31.5px; /* Reduced from 35px to 90% */
  pointer-events: none;
  z-index: 9999;
...
const CursorGlow = styled.div.attrs({ className: 'cursor-glow' })`
  ...
  box-shadow: 0 0 16px 3.6px var(--lavender-light), 0 0 22.5px 9px var(--lavender-dark); /* Reduced shadow sizes to 90% */
